...
function matchesFigureSkating(summary?: string): boolean {
  const s = (summary ?? "").toLowerCase().trim();
  if (!s) return false;
  return FIGURE_SKATING_KEYWORDS.some((k) => s.includes(k));
}
...
export async function fetchCalendarEvents(): Promise<CalendarEvent[]> {
  ...
  try {
    ...
    const events: CalendarEvent[] = items
      ...
      .filter((item) => matchesFigureSkating(item.summary))
      .map((item) => {
